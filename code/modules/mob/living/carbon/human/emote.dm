...
//New emotes added
		...
		if("rolleyes")
			message = "<B>[src]</B> rolls their eyes."
			m_type = 1
		...
		if("smoke1")
			message = "<B>[src]</B> takes a long, deep drag of their cigarette."
			m_type = 2

		if("smoke2")
			message = "<B>[src]</B> removes their cigarette, exhaling a large white puff of smoke into the air."
			m_type = 2

		if("chin")
			message = "<B>[src]</B> scratches their chin."
			m_type = 2

		if("flutter")
			message = "<B>[src]</B> flutters their eyelashes, batting them flirtily."
			m_type = 2

		if("scowl")
			message = "<B>[src]</B> scowls horribly."
			m_type = 2


		if("scowl2")
			message = "<B>[src]</B> displays one mean scowl!"
			m_type = 2
		...
		if ("lookdown")
			...
			if (param)
				message = "<B>[src]</B> looks up and down at [param], inspecting them with a judgemental look on their face."
			else
				message = "<B>[src]</B> has a judgemental expression on their face."
		...
		if("sidestep")
			message = "<B>[src]</B> does a few concise sidesteps, dancing rhythmically."
			m_type = 2

		if ("whistle")
			if (miming)
				message = "<B>[src]</B> acts out a whistle!"
				m_type = 1
			else
				if (!muzzled)
					var/sound = "sound/effects/whistle.wav"
					if(gender == FEMALE)
						sound = "sound/effects/whistle.wav"
					playsound(src.loc, sound, 50, 1, 10, 1.2)
					message = "<B>[src]</B> whistles!"
					src.adjustOxyLoss(5)
					m_type = 2
				else
					message = "<B>[src]</B> makes an alternating vocal pattern."
					m_type = 2
